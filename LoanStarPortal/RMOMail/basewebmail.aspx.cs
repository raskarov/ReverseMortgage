...
namespace WebMailPro
{
	...
	public partial class basewebmail : System.Web.UI.Page
	{
		...
		protected MessageContainer Control_Messagecontainer;
		protected Folders_part Control_FoldersPart;
		protected Inbox Control_Maillist;
		protected Toolbar Control_Toolbar;
		protected Copyright Control_Copyright;
		...
		protected void Page_Load(object sender, EventArgs e)
		{
            ...
            if (Request.QueryString.Get("scr") != null)
            {
                ...
                string scr = Request.QueryString.Get("scr"); 
                if(scr=="followup"&& Session["condition_message"]!= null)
                {
...
");
		...
		protected void ShowContacts()
		{
			...
			if (ctrlContacts != null)
			{
				...
				{
					sortField = ((HFAction.Value == "sort") && (HFRequest.Value == "contacts")) ? short.Parse(this.HFValue.Value) : short.Parse(Session["contacts_sort_field"].ToString());
		...
		protected void AddContactToGroup()
		{
			try
			{
				...
				BaseWebMailActions baseAction = new BaseWebMailActions(acct, Page);
...
//			ctrlContacts.LoadContactsGroups(acct);
		...
		protected void SortMails()
		{
			try
			{
				...
				{
					case 10:
						if(acct.DefaultOrder != (DefaultOrder) Convert.ToInt32(HFValue.Value))
						{
							SortOrder = 0;
							SortField = 10;
						}
				...
				acct.DefaultOrder = (DefaultOrder) (SortOrder + SortField);
				acct.Update(false);
		...
		protected void SendMsg()
		{
			try
			{
                ...
                string From = Request.Form[ctrlNewMessage.FindControl("fromTextBox").UniqueID];
                string To = Request.Form[ctrlNewMessage.FindControl("toTextBox").UniqueID];
                string Cc = Request.Form[ctrlNewMessage.FindControl("ccTextBox").UniqueID];
                string Bcc = Request.Form[ctrlNewMessage.FindControl("bccTextBox").UniqueID];
                string Subject = Request.Form[ctrlNewMessage.FindControl("subjectTextBox").UniqueID];
				...
				for(int i = 0; Request.Form.AllKeys.Length > i; i++)
				{
					if(Request.Form.AllKeys[i].StartsWith("attachments["))
					{
						...
						AttachmentTempName = Path.Combine(Utils.GetTempFolderName(Session), AttachmentTempName);
                ...
                if (Session["condition_message"] != null)
                {
                    CloseEmail();
                    return;
                }
...
",
        ...
        {
            Response.Redirect("closeemail.aspx");
        }
        ...
        private int GetCondition(string name)
        {
            int res = -1;
	    ...
	    private void AssociateMessage(WebMailMessage msg)
        {

            Object o = Session["assosiationtype"];
            if(o!=null)
            {
                int assosiationType = 0;
                try
                {
                    assosiationType = int.Parse(o.ToString());
                }
                catch
                {
                }
                int conditionId = -1;
                if(assosiationType == 1)
                {
                    conditionId = GetCondition("ConditionID");
                }
                else if(assosiationType == 2)
                {
                    conditionId = GetCondition("CampaignId");
                }
                if(conditionId>0)
                {
                    o = Session["condition_message"];
                    if(o!=null)
                    {
                        try
                        {
                            bool b = bool.Parse(o.ToString());
                            if (b)
                            {
                                msg.AssociationType = assosiationType;
                                msg.ConditionId = conditionId;
                                msg.LoanAppIDs = Session["MortgageProfileID"].ToString();
                            }
                        }
                        catch
                        {
