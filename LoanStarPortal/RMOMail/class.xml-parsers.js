...
CMessage.prototype = {
	...
	PrepareForEditing: function (msg)
	{
	    ...
	    this.LoanApplicants = HtmlDecode(msg.LoanApplicants);
		this.LoanAppIDs = HtmlDecode(msg.LoanAppIDs);
	...
	GetInXML: function()
	{
		...
		strHeaders += '<loanapplicants>' + GetCData(this.LoanApplicants) + '</loanapplicants>';
		strHeaders += '<loanappids>' + GetCData(this.LoanAppIDs) + '</loanappids>';
		var strGroups = '';
		...
		var attrs = '';
		var uid = '';
		var folder = '<folder>' + this.FolderFullName + '</folder>';
		if (this.Id != -1) {
			attrs += ' id="' + this.Id + '"';
		...
		strResult = '<message' + attrs + '>' + uid + folder + strHeaders + strBody + strAttachments + '</message>';
						...
						for (var j=0; j<HeadersParts.length; j++) {
							...
							if (part_.length > 0) {
								switch (HeadersParts[j].tagName) {
									...
									break;
									case 'loanapplicants':
										this.LoanApplicants = Trim(part_[0].nodeValue);
									break;
									case 'loanappids':
										this.LoanAppIDs = Trim(part_[0].nodeValue);
									break;
									case 'date':
	...
	UpdateAssociation: function(names,ids)
	{
	    this.LoanApplicants = names;
	    this.LoanAppIDs = ids;
	}
};
			...
			if (part.length > 0) {
				switch (HeadersParts[i].tagName) {
					...
					case 'subject':
						...
						break;
					case 'loanapplicants':
						this.LoanApplicants = Trim(part[0].nodeValue);
						break;
					case 'loanappids':
						this.LoanAppIDs = Trim(part[0].nodeValue);
						break;
					case 'date':
