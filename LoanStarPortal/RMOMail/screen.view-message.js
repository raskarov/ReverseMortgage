...
CViewMessageScreen.prototype = {
	...
	{
//	    if(this.FolderFullName == 'Trash')
//	        this._linkloanButton.style.display = 'none';
//	    else     
//	        this._linkloanButton.style.display = '';
	...
	Fill: function (msg)
	{
		...
		this._cLoanObj.innerHTML = msg.LoanApplicants;
		this._cLoanAppIdsObj.value = msg.LoanAppIDs;
...
//		this._linkloanButton.onclick = this.SaveChanges(msg);
	...
	Build: function (container, accountsBar, PopupMenus, settings)
	{
		...
		td.className = 'wm_view_message_title';
		td.innerHTML = 'Loan:';
		WebMail.LangChanger.Register('innerHTML', td, 'Loan', ':');
		td = tr.insertCell(1);
		img = CreateChildWithAttrs(td, 'img', [['class', 'wm_importance_img']]);
		this._importanceImg = img;
		span = CreateChild(td, 'span');
		span.id = 'loanAppNamesView';
		this._cLoanObj = span;

		td = tr.insertCell(2);
		inp = CreateChildWithAttrs(td, 'input', [['type', 'hidden'], ['id', 'loanAppIDsView'],  ['maxlength', '255']]);
		this._cLoanAppIdsObj = inp;
...
//		this._linkloanButton = toolBar.AddItem(TOOLBAR_SAVELOAN_IDs, function () {});
    ...
    SaveChanges: function (msg)
	{
	   return function () {
	        var test = '';
		    var saveMsg = new CMessage();
		    saveMsg.FromAddr = msg.FromAddr;
		    saveMsg.ToAddr = msg.ToAddr;
		    saveMsg.CCAddr = msg.CCAddr;
		    saveMsg.BCCAddr = msg.BCCAddr;
		    saveMsg.Subject = msg.Subject;;
		    saveMsg.Importance = msg.Importance;
		    saveMsg.LoanApplicants = document.getElementById('loanAppNamesView').innerText;
		    saveMsg.LoanAppIDs = document.getElementById('loanAppIDsView').value;
    		
		    saveMsg.HasHtml = msg.HasHtml;
    	    
	        if (saveMsg.HasHtml) {
			    saveMsg.HtmlBody = msg.HtmlBody;
		    }
		    else {
			    saveMsg.PlainBody = msg.PlainBody;
		    };

		    saveMsg.Attachments = msg.Attachments;
		    saveMsg.Id = msg.Id.toString();
		    saveMsg.Uid = msg.Id.toString();
		    saveMsg.SendersGroups = msg.SendersGroups;
		    saveMsg.FolderId = msg.FolderId;
		    saveMsg.FolderFullName = msg.FolderFullName;
		    
		    var xml = saveMsg.GetInXML();
...
CMessageViewer.prototype = {
	...
	Fill: function (msg)
	{
		...
		window.parent.ShowEmailLinks(argsSelectedMenuItem, msg);
