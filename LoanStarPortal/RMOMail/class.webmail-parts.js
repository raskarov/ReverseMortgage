...
var argsSelectedMenuItem = null;
var selectedMailMode = null;

/* Handlers */
function SetHistoryHandler(args)
{
   argsSelectedMenuItem = args;
    
   //back to list
   if(args.ScreenId == 1 && args.FolderId == null){
      window.parent.ShowEmailLinks(args);
      selectedMailMode = 'Default';
   }
   //new message   
   else if(args.ScreenId == 4){
      window.parent.ShowEmailLinks(args);
      selectedMailMode = 'NewMessage';
   }
   //open message
   else if(args.ScreenId == 3){
      selectedMailMode = 'OpenMessage';
   }
   //preview
   else if(args.ScreenId == 1 && args.FolderId > 0 && args.IdAcct > 0){
       selectedMailMode = '';
   }
   //default
   else {
