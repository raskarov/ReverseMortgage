...
function CToolBar(parent, skinName)
{
	...
	this._descriptions[TOOLBAR_SAVELOAN_IDs] = {
		title: ' Link Loan',
		imgFile: 'MailLink.gif',
		imgClass: 'wm_menu_print_message_img',
		langField: 'LinkLoan'
	};
...
// RMENGAGE		
        if(this._name=='Inbox')
        {
            var l_=parent.document.getElementById('emailLinkId');
            if(l_!=null){
                l_ = parent.document.getElementById(l_.value);
                if(l_){
                    if(this._newMsgsCount>0){
                        l_.innerHTML = 'Emails('+this._newMsgsCount+')';
                        l_.className='text_red';
                    }else{
                        l_.innerHTML = 'Emails';
                        l_.className='text';
                    }
