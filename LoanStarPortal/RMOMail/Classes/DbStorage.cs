...
namespace WebMailPro
{
	...
	public abstract class DbStorage	: MailStorage, IDisposable
	{
		...
		{
			return _dbMan.SelectAddressBookContact(id_addr, null);
		}

		public AddressBookContact GetAddressBookContact(string email)
		{
			return _dbMan.SelectAddressBookContact(-1, email);
		}
		...
		public Filter CreateFilter(byte field, byte condition, string filter, byte action, long id_folder)
		{
			return _dbMan.CreateFilter(_account.ID, field, condition, filter, action, id_folder);
		}

		public void UpdateFilter(int id_filter, byte field, byte condition, string filter, byte action, long id_folder)
		{
			_dbMan.UpdateFilter(id_filter, _account.ID, field, condition, filter, action, id_folder);
		}
		...
		public virtual WebMailMessageCollection LoadMessagesByUids(string[] strUids, Folder fld)
		{
			return _dbMan.SelectMessages(strUids, fld);
		}
		...
		public virtual WebMailMessageCollection LoadMessagesToDelete(bool allMessages, object[] messageIndexSet, Folder fld)
		{
			if (!allMessages)
			{
				...
				{
					return this.LoadMessages(messageIndexSet, fld);
				}
				return this.LoadMessages(fld);
			}
			else
			{
		...
		{
			_dbMan.AlterTable(tableName, prefix);
		}
