	...
	public abstract class DbManager : IDisposable
	{
		...
		public virtual int GetAccountsCount(string condition)
		{
			...
			try
			{
				...
				IDbCommand command = _commandCreator.SelectAwmAccountsCount(condition);
				command.Transaction = myTrans;
				object obj = ExecuteScalarCommand(command);
				myTrans.Commit();
				...
				if (updateUser)
				{
					if (acct.UserOfAccount != null)
					{
						...
						if (acct.UserOfAccount.Columns != null)
						{
							...
							existingColumns = (UserColumn[])list.ToArray(typeof(UserColumn));
							...
							foreach (UserColumn column in acct.UserOfAccount.Columns)
							{
								...
								if (needToInsert)
								{
									...
									object obj = ExecuteScalarCommand(command);
									long last_id = (obj != null) ? Convert.ToInt64(obj) : -1;
								}
								if (needToUpdate)
								{
		...
		public virtual DataView SelectAccountsAdminReader(int page, int pageSize, string orderBy, bool asc, string searchCondition)
		{
			IDbCommand command = _commandCreator.SelectAwmAccountsForAdmin(page, pageSize, orderBy, asc, searchCondition);
			IDataAdapter adapter = CreateDataAdapter(this, command);
			DataSet ds = new DataSet();
			adapter.Fill(ds);

			DataTable dt = ds.Tables[0];
			dt.Columns.Add("all_mailboxs_size", typeof(int));
			foreach (DataRow row in dt.Rows)
			{
				int id_user = Convert.ToInt32(row["id_user"]);
				row["all_mailboxs_size"] = GetUserMailboxsSize(id_user);
			}
		...
		{
			IDbCommand command = _commandCreator.SelectAwmAccounts(id_user, -1);
			return command.ExecuteReader();
		}

		public virtual Account[] SelectUserAccounts(int id_user)
		{
		...
		public virtual Account SelectAccountData(string email, string login, string password)
		{
			...
			try
			{
				...
				IDbCommand command = _commandCreator.SelectAwmAccounts(email, login, Utils.EncodePassword(password));
				command.Transaction = myTrans;
				using (IDataReader dataReader = command.ExecuteReader())
				{
					if (dataReader.Read())
		...
		public virtual Account SelectAccountData(int id_acct, int id_user)
		{
			Account acct = null;
			IDbTransaction myTrans = null;
			try
			{
				...
				IDbCommand command = _commandCreator.SelectAwmAccounts(id_user, id_acct);
				command.Transaction = myTrans;
				using (IDataReader dataReader = command.ExecuteReader())
				{
					if (dataReader.Read())
		...
		public virtual AddressBookContact SelectAddressBookContact(long id_addr, string email)
		{
			...
			try
			{
				...
				IDbCommand command = (email == null) ? _commandCreator.SelectAwmAddrBook(_account.UserOfAccount.ID, id_addr) 
					: _commandCreator.SelectAwmAddrBook(_account.IDUser, Utils.ConvertToDBString(_account, email));
				command.Transaction = myTrans;
				using (IDataReader reader = command.ExecuteReader())
				{
					if (reader.Read())
					{
						contact = ReadAddressBookContact(reader);
					}
				}
				if (contact != null)
				{
		...
		public virtual int GetAddressBookContactsCount(int id_user, string look_for, int look_for_type)
		{
			...
			try
			{
				...
				IDbCommand command = _commandCreator.SelectAddressBookContactsCount(id_user, look_for, look_for_type);
				command.Transaction = myTrans;
				object obj = ExecuteScalarCommand(command);
				myTrans.Commit();
				result = (obj != null) ? Convert.ToInt32(obj) : -1;
		...
		public virtual int GetAddressBookGroupsCount(int id_user, string look_for, int look_for_type)
		{
			...
			try
			{
				...
				IDbCommand command = _commandCreator.SelectAddressBookGroupsCount(id_user, look_for, look_for_type);
				command.Transaction = myTrans;
				object obj = ExecuteScalarCommand(command);
				myTrans.Commit();
				result = (obj != null) ? Convert.ToInt32(obj) : -1;
		...
		public virtual void SaveMessage(int id_msg, WebMailMessage message, Folder folder)
		{
			...
			if ((message.BodyText == null)||(message.BodyText.Length == 0))
			{
				...
				{
					if ((message.MailBeeMessage.BodyPlainText == null) || (message.MailBeeMessage.BodyPlainText.Length == 0))
					{
			...
			try
			{
				...
				IDbCommand command = _commandCreator.InsertIntoAwmMessages( 
                    ...
                    message.StrUid, 
                    message.IntUid, 
                    Utils.ConvertToDBString(_account, message.FromMsg.ToString()), 
                    Utils.ConvertToDBString(_account, message.ToMsg.ToString()), 
                    Utils.ConvertToDBString(_account, message.CcMsg.ToString()), 
                    Utils.ConvertToDBString(_account, message.BccMsg.ToString()), 
                    Utils.ConvertToDBString(_account, message.Subject),
                    //Utils.ConvertToDBString(_account, message.LoanApplicants),
                    //Utils.ConvertToDBString(_account, message.LoanAppIDs),
                    //message.ConditionId,
                    message.MsgDate, 
                    message.Attachments, 
                    message.Size, 
                    message.Seen, 
                    message.Flagged, 
                    Convert.ToByte(message.Priority), 
                    message.Downloaded, 
                    message.XSpam, 
                    message.Rtl, 
                    message.Deleted, 
                    ...
                    message.Replied, 
                    message.Forwarded, 
                    Convert.ToByte(message.Flags), 
                    Utils.ConvertToDBString(_account, message.BodyText), 
                    message.Grayed, 
                    message.OverrideCharset);
                ...
                if (!String.IsNullOrEmpty(message.LoanAppIDs))
                {
                    WebMailHelper.AddLoanAssosiation(id_msg, message.LoanAppIDs);
                }
                if(message.ConditionId>0)
                {
                    ...
                    try
                    {
                        mortgageId = int.Parse(message.LoanAppIDs);
                    }
                    catch
                    {
                    }
                    WebMailHelper.AddEmailAssociation(id_msg, message.AssociationType , message.ConditionId,mortgageId);
                }
			}
			catch (Exception ex)
			{
		...
		public virtual WebMailMessage SelectMessage(int id_msg, Folder fld)
		{
			...
			try
			{
			    ...
			    command = _commandCreator.SelectLoansAssociation(id_msg);
                command.Transaction = myTrans;
			    string loanapps = String.Empty;
			    string loanids = String.Empty;
                using (IDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        loanids += reader.GetInt32(1).ToString() + ";";
                        loanapps += reader.GetString(2) + ";";
                    }
                }
			    webMsg.LoanAppIDs = loanids;
			    webMsg.LoanApplicants = loanapps;
				MailMessage message = LoadMailMessage(command, myTrans, webMsg, fld);
				if (message != null)
				{
					webMsg.Init(message, ((webMsg.StrUid != null) && (webMsg.StrUid.Length > 0)), fld);
				}
		...
		protected MailMessage LoadMailMessage(IDbCommand command, IDbTransaction transaction, WebMailMessage webMsg, Folder fld)
		{
			if ((_account != null) && (webMsg != null) && (command != null) && (fld != null))
			{
				if (webMsg.Downloaded)
				{
					...
					{
						command = _commandCreator.SelectAwmMessagesBody(_account.ID, webMsg.IDMsg);
		...
		public virtual WebMailMessageCollection SearchMessages(int page, string condition, FolderCollection folders, bool inHeadersOnly, out int searchMessagesCount)
		{
			...
			try
			{
				...
				if (obj != null)
				{
					searchMessagesCount = Convert.ToInt32(obj);
				}
				else
				{
					searchMessagesCount = messageCollection.Count;
				}
		...
		public virtual void UpdateMessage(WebMailMessage message)
		{
			...
			try
			{
				...
				IDbCommand command = _commandCreator.UpdateAwmMessages( 
                    message.ID, 
                    message.IDMsg, 
                    message.IDAcct, 
                    message.IDFolderSrv, 
                    ...
                    message.StrUid, 
                    message.IntUid, 
                    Utils.ConvertToDBString(_account, (message.FromMsg != null) ? message.FromMsg.ToString() : string.Empty), 
                    Utils.ConvertToDBString(_account, (message.ToMsg != null) ? message.ToMsg.ToString() : string.Empty), 
                    Utils.ConvertToDBString(_account, (message.CcMsg != null) ? message.CcMsg.ToString() : string.Empty), 
                    Utils.ConvertToDBString(_account, (message.BccMsg != null) ? message.BccMsg.ToString() : string.Empty), 
                    Utils.ConvertToDBString(_account, message.Subject),
                    Utils.ConvertToDBString(_account, message.LoanApplicants),
                    Utils.ConvertToDBString(_account, message.LoanAppIDs),
                    message.MsgDate, 
		...
		public virtual WebMailMessageCollection SelectMessages(string[] str_uids, Folder fld)
		{
			IDbTransaction myTrans = null;
			WebMailMessageCollection messageCollection = new WebMailMessageCollection();
			try
			{
				...
				IDbCommand command = _commandCreator.SelectAwmMessages(_account.ID, fld.ID, str_uids);
				command.Transaction = myTrans;
				using (IDataReader reader = command.ExecuteReader())
				{
					while(reader.Read())
		...
		public virtual Filter CreateFilter(int id_acct, byte field, byte condition, string filter, byte action, long id_folder)
		{
			IDbTransaction myTrans = null;
			try
			{
				...
				IDbCommand command = _commandCreator.InsertIntoAwmFilters(id_acct, field, condition, Utils.ConvertToDBString(_account, filter), action, id_folder);
				command.Transaction = myTrans;
				object obj = ExecuteScalarCommand(command);
				myTrans.Commit();
				...
				return new Filter(id_filter, id_acct, (FilterField)field, (FilterCondition)condition, filter, (FilterAction)action, id_folder);
			}
			catch(Exception ex)
			{
				if (myTrans != null) myTrans.Rollback();
		...
		public virtual void UpdateFilter(int id_filter, int id_acct, byte field, byte condition, string filter, byte action, long id_folder)
		{
			IDbTransaction myTrans = null;
			try
			{
				myTrans = _connection.BeginTransaction();
				IDbCommand command = _commandCreator.UpdateAwmFilters(id_filter, id_acct, field, condition, Utils.ConvertToDBString(_account, filter), action, id_folder);
				command.Transaction = myTrans;
				command.ExecuteNonQuery();
				myTrans.Commit();
			}
		...
		public virtual void AlterTable(string tableName, string prefix)
		{
			IDbTransaction myTrans = null;
			try
			{
				...
				IDbCommand command = _commandCreator.AlterTable(tableName, prefix);
				command.Transaction = myTrans;
				command.ExecuteNonQuery();
		...
		public virtual void CreateDomain(string name, short mail_protocol, string mail_inc_host, int mail_inc_port, string mail_out_host, int mail_out_port, bool mail_out_auth)
		{
			IDbTransaction myTrans = null;
			try
			{
				myTrans = _connection.BeginTransaction();
				IDbCommand command = _commandCreator.InsertIntoAwmDomains(name, mail_protocol, mail_inc_host, mail_inc_port, mail_out_host, mail_out_port, mail_out_auth);
				command.Transaction = myTrans;
				command.ExecuteNonQuery();
				myTrans.Commit();
			}
		...
		public virtual DataView SelectDomains(int page, int pageSize, string orderBy, bool asc, string searchCondition)
		{
        ...
        public virtual void UpdateDomain(int id_domain, string name, short mail_protocol, string mail_inc_host, int mail_inc_port, string mail_out_host, int mail_out_port, bool mail_out_auth)
		{
			IDbTransaction myTrans = null;
			try
			{
				myTrans = _connection.BeginTransaction();
				IDbCommand command = _commandCreator.UpdateAwmDomains(id_domain, name, mail_protocol, mail_inc_host, mail_inc_port, mail_out_host, mail_out_port, mail_out_auth);
				command.Transaction = myTrans;
				command.ExecuteNonQuery();
				myTrans.Commit();
			}
				...
				if (fieldName != null)
				{
					switch (fieldName.ToLower(CultureInfo.InvariantCulture))
					{
						...
						case "subject":
							...
							break;
                        case "loanapplicants":
                            webMsg.LoanApplicants = Utils.ConvertFromDBString(_account, reader.GetString(index));
                            break;
                        case "loanappids":
                            webMsg.LoanAppIDs = Utils.ConvertFromDBString(_account, reader.GetString(index));
                            break;
                        case "msg_date":
						...
						case "forwarded":
							...
							break;
						case "flags":
							webMsg.Flags = (SystemMessageFlags)reader.GetByte(index);
							break;
						case "body_text":
						...
						case "grayed":
							...
							break;
						case "folder_name":
							webMsg.FolderFullName = Utils.ConvertFromDBString(_account, reader.GetString(index));
							break;
						case "charset":
...
//                                settings.DefaultTimeFormat = TimeFormats.F24;
		...
		internal static IDataAdapter CreateDataAdapter(DbManager manager, IDbCommand command)
		{
			...
			else if (manager is MsAccessDbManager)
			{
				return new OleDbDataAdapter((OleDbCommand)command);
			}
			else if (manager is MySqlDbManager)
			{
	...
	{
		public MsSqlDbManager() : this(null) {}
