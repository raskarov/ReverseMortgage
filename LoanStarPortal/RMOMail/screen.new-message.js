...
CNewMessageScreen.prototype = {
	...
	Fill: function ()
	{
		if ((null != this._msgObj) && this.shown) {
	        ...
	        this._loanObj.value   = msg.LoanApplicants;
	        this._loanAppIdsObj.value   = msg.LoanAppIDs;
...
//mode = 1 - save message
	SaveChanges: function (mode)
	{
		...
		var loanApplicantsValue =  this._loanObj.value;
		var loanAppIDsValue =  this._loanAppIdsObj.value;
		...
		if (save_anyway) {
			...
			newMsg.LoanApplicants = loanApplicantsValue;
			newMsg.LoanAppIDs = loanAppIDsValue;
		    ...
		    newMsg.FolderFullName = this._msgObj.FolderFullName;
	...
	Build: function (container, accountsBar, popupMenus, settings)
	{
		...
		td = tr.insertCell(1);
		inp = CreateChildWithAttrs(td, 'input', [['type', 'text'], ['id', 'subjectNewMsg'], ['class', 'wm_input'], ['size', '93'], ['maxlength', '255']]);
		inp.tabIndex = 5;
		inp.onfocus = function () {
		...
		td = tr.insertCell(1);
		inp = CreateChildWithAttrs(td, 'input', [['type', 'text'], ['id', 'loanAppNames'],  ['readonly', 'readonly'], ['class', 'wm_input'], ['size', '93'], ['style', 'border-style:none; background-color:#dbeaf5;'], ['maxlength', '255']]);
		inp.tabIndex = -1;
		this._loanObj = inp;

		tr = tbl.insertRow(RowIndex++);
		td = tr.insertCell(0);
		inp = CreateChildWithAttrs(td, 'input', [['type', 'hidden'], ['id', 'loanAppIDs'],  ['maxlength', '255']]);
		inp.tabIndex = -1;
		this._loanAppIdsObj = inp;
